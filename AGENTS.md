...
- `game.js` — entire game: input, `Bullet`/`Asteroid`/`Ship`/`Particle`/`PowerUp` classes, game state, `update()`/`draw()`, `requestAnimationFrame` loop. Runs top-level; no modules, no exports.
...
- Game tuning lives in per-size lookup tables at the top of the Asteroid section (`RADII`, `SPEEDS`, `POINTS`), constants inside `Ship.update()` (`ROT`, `THRUST`, `DRAG`), and power-up constants (`DROP_CHANCE`, `SPEED_DURATION`) at the PowerUp section.
